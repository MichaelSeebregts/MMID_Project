...
N = 12
B = 14 + 10*3 + 3  # number of variables per patch
A = 21  + # Transitions for Child
    13*3 + # Transitions for Men, Non Pregnant Women and Pregnant Women
    ((14 + 10*3)*(N)) + # Migration 
    (14 + 10*3) + # Deaths
    (14 + 14 + 10*2) + # Aging for children and women moving from not pregnant to pregnant 
...
compartmentsChildren = c("S", "It", "V1", "V2", "V3", "E", "C", "U", "A", "Tc", "NotTc", "Tu", "NotTu", "R") 
compartmentsOther = c("S", "E", "C", "U", "A", "Tc", "NotTc", "Tu", "NotTu", "R")
...
for (n in 1:N) {
  ...
  # Birth
  transitions[traind[1 + indAdd,n],] <- c(varind[1,n],  0, varind[2,n], +1) #    -> IT
  indAdd = indAdd +1
  
  # Loss of immunity
  transitions[traind[2 + indAdd,n],] <- c(varind[2,n],  -1, varind[1,n], +1) #  IT  -> S
  indAdd = indAdd + 1
  # V1 
  ...
  transitions[traind[26 + indAdd,n],] <- c(varind[86,n],  -1, varind[87,n], 1) # S -> E
  transitions[traind[29 + indAdd,n],] <- c(varind[89,n],  -1, varind[90,n], 1) # E -> I
